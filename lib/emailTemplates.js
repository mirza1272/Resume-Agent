...
function detectRole(jobDesc) {
  const text = jobDesc.toLowerCase()

  // ── Web / Frontend roles (check before AI to avoid false positives) ──
  if (/full[\s-]*stack\s*(developer|engineer|internship|intern|role)/i.test(text)) {
    return 'FULL_STACK'
  }
  if (/front[\s-]*end\s*(developer|engineer|internship|intern|role)/i.test(text)) {
    return 'FRONTEND'
  }
  if (/\bweb\s+(developer|engineer|internship|intern|role)/i.test(text) &&
    !/full[\s-]*stack|front[\s-]*end|back[\s-]*end/.test(text)) {
    return 'WEB_DEVELOPER'
  }

  // ── AI / ML roles ────────────────────────────────────────────────────
  if (/machine\s*learning\s*(engineer|internship|intern|role)/i.test(text) && !/\bai\b/.test(text)) {
    return 'ML_ENGINEER'
  }
  if (/\bai\s*\/?\s*ml\s*(engineer|internship|intern|role)/i.test(text)) {
    return 'AI_ML_ENGINEER'
  }
  if (/\bai\s+(engineer|internship|intern|role)/i.test(text) && !/machine\s*learning/.test(text)) {
    return 'AI_ENGINEER'
  }
  if (/machine\s*learning/i.test(text)) {
    return 'ML_ENGINEER'
  }
  if (/artificial\s*intelligence|deep\s*learning|llm|generative\s*ai|computer\s*vision/i.test(text)) {
    return 'AI_ML_ENGINEER'
  }

  // ── Software Engineer roles ──────────────────────────────────────────────────
  if (/software\s*(engineer|engineering)\s*(intern|internship)/i.test(text)) {
    return 'SE_INTERN'
  }
  if (/software\s*(engineer|engineering|developer)/i.test(text)) {
    return 'SE'
  }
...
function extractCompany(jobDesc) {
  // Try common patterns like "at Acme Corp" / "@ Acme" / "- Acme Inc"
  const patterns = [
...
export function generateEmail(jobDesc) {
  ...
  const templateFn = TEMPLATES[role] || TEMPLATES.AI_ML_ENGINEER
  return templateFn(company)
}
