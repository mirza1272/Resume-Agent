...
function detectRolePipeline(jobDesc) {
  ...
  // STEP 1: Extract Job Title (Look at first few lines for obvious title)
  let title = ''
  ...
  const typeMatch = text.match(/\b(internship|intern|trainee|associate|junior|part-time|part time|contract)\b/)
  if (typeMatch) {
  ...
  else if (titleLower.match(/\bai\b/) && titleLower.includes('engineer')) role = 'AI_ENGINEER'
  ...
  else if (titleLower.match(/\bweb\b/) && titleLower.includes('developer')) role = 'WEB_DEVELOPER'
...
export function generateEmail(jobDesc) {
  const templateKey = detectRolePipeline(jobDesc)
  const company = extractCompany(jobDesc)
  const templateFn = TEMPLATES[templateKey] || TEMPLATES.AI_ML_ENGINEER
